...
def _class_names() -> tuple[str, ...]:
    return tuple(target.label for target in COMMON_CANCER_TARGETS)


def _samples_for(root: Path, split: str) -> list[tuple[Path, int]]:
    from medical.classifier import iter_medical_image_paths

    return [
        (img, class_index)
        for class_index, class_name in enumerate(_class_names())
        for img in iter_medical_image_paths(root / class_name / "processed" / "images" / split)
    ]


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Train 8 ung thu (bao gom nao) - cau hinh cao cho GPU 4GB VRAM."
    )
...
def run_8cancers_training(args) -> int:
    ...
    val_samples = _samples_for(root, "val")
    if args.max_train_samples:
        train_samples = train_samples[: args.max_train_samples]
    if not train_samples:
        raise FileNotFoundError("Khong co du lieu train 8 ung thu.")

    class_weights = _compute_class_weights(train_samples, class_names)

    ckpt_dir = Path("output/medical/checkpoints")
    ckpt_dir.mkdir(parents=True, exist_ok=True)
    ckpt_path = ckpt_dir / "medical_8_cancers_cnn.pt"

    print(
        f"[2/3] Train CNN 8 lop: backbone=convnext_tiny(pretrained) image={args.image_size}x{args.image_size} "
    ...
    target_path = Path("medical_8_cancers_cnn.pt")
...
def launch_detached(args) -> int:
    ...
    cmd = [sys.executable, "run_train_8cancers_high.py"]
    env = dict(os.environ)
    ...
    print("Da khoi dong train 8 ung thu o che do detached.")
...
def main() -> int:
    ...
    try:
        code = run_8cancers_training(args)
