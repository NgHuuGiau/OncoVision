...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--metadata-output", default="dataset/medical/skin_lesion/metadata/isic2016_import.csv")
...
def main() -> None:
    ...
    print(row("Target raw images", str(config.raw_images_dir), GREEN, bounded=False))
    print(row("Target raw labels", str(config.raw_labels_dir), GREEN, bounded=False))
    print(line(rule("-"), CYAN))
    result = import_isic_2016_part3b_to_yolo(
        args.source_root,
        target_images_dir=config.raw_images_dir,
        target_labels_dir=config.raw_labels_dir,
    ...
    print(row("Metadata", args.metadata_output, GREEN, bounded=False))
    print(line(rule("-"), CYAN))
