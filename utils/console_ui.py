...
def _render_prompt(hardware: Any | None = None, recommendations: dict[str, Any] | None = None, print_fn=print) -> None:
    ...
    lines = [
        _line(_rule("="), CYAN),
        _line(_pad("OncoVision CAMERA : CHON CẤU HÌNH CHAY"), BOLD + CYAN),
        _line(_rule("="), CYAN),
...
def prompt_launch_target(
    *,
    ...
    while True:
        ...
        lines = [
            _line(_rule("="), CYAN),
            _line(_pad("OncoVision CAMERA :: CHỌN KIỂU KHỞI ĐỘNG"), BOLD + CYAN),
            _line(_rule("="), CYAN),
            ...
            _row("1 | GIAO DIEN", "Mở giao diện desktop / chat.", GREEN),
...
def explain_runtime_failure(error: Exception) -> tuple[str, list[str], list[str]]:
    ...
    if "khong khoi tao duoc ultralytics" in lower_message or "khong khoi tao duoc detector" in lower_message:
        return (
            "Không nạp được OncoVision / ultralytics để bắt đầu nhận diện.",
            [
